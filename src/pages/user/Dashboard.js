  ...
  const openPopup = async () => {
    try {
      ...
      const response = await axios.get(
        `https://pokemonapp-fwmi.onrender.com/api/v1/poke/pokedetail?email=${auth?.user?.email}`
      );
                      ...
                      <div className="mainClass">
                        {userData.map((user, index) => (
                          <div className="mypokocard" key={index}>
                            <img src={`https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/other/dream-world/${user.id}.svg`} alt="" />
